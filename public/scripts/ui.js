...
const ChatPanel = (function () {
  ...
  const updateTyping = function (typingUser) {
    if (typingUser.username !== Authentication.getUser().username) {
      $('#typing-indicator').text(`${typingUser.name} is typing...`);
  ...
  return { initialize, update, addMessage, updateTyping };
})();
...
const UI = (function () {
  ...
  const getUserDisplay = function (user) {
    return $("<div class='field-content row shadow'></div>")
      .append(
        $(
          "<span class='user-avatar'>" + Avatar.getCode(user.avatar) + '</span>'
        )
      )
